...
fn emit_generated_rs(repo_root: &Path, out_path: &Path, gen: &GenConfig) -> std::io::Result<()> {
    ...
    // Short aliases to reduce verbosity in emitted code for this repo
    if gen.const_root == "ratatui" {
    ...
    for sd in &structs {
        ...
        if all_str && sd.name == "Set" {
            ...
            for (mod_path, consts) in grouped {
                ...
                writeln!(f, "{}({}: {});", m_struct_str, ffi_name, fields)?;
                let last = mod_path.split("::").last().unwrap_or("set");
                for (cname, _file) in consts {
                    let full = format!("{}::{}::{}", const_prefix, mod_path.replace('/', "::"), cname);
                    let fn_name = format!(
                        "{}_symbols_{}_get_{}",
                        gen.fn_prefix,
                        last,
                    ...
                    writeln!(
                        f,
                        "{}({}, {}, {} , [{}]);",
                        m_const_struct_getter, fn_name, ffi_name, full, fields
                    )?;
                }
            }
        ...
        if all_str {
            ...
            if let Some(items) = by_type_consts.get(&sd.name) {
                writeln!(f, "{}({}: {});", m_struct_str, ffi_name, field_list)?;
                for (cname, file) in items {
                    let mod_path = rel_module_path(&base_src, file);
                    let full = if mod_path.is_empty() {
                        format!("{}::{}", const_prefix, cname)
                    } else {
                        format!("{}::{}::{}", const_prefix, mod_path.replace('/', "::"), cname)
                    };
                    ...
                    writeln!(
                        f,
                        "{}({}, {}, {} , [{}]);",
                        m_const_struct_getter, fn_name, ffi_name, full, field_list
                    )?;
                }
            }
        } else if all_color {
            if let Some(items) = by_type_consts.get(&sd.name) {
                ...
                writeln!(f, "{}({}: {});", m_struct_u32, ffi_name, field_list)?;
                for (cname, file) in items {
                    let mod_path = rel_module_path(&base_src, file);
                    let full = if mod_path.is_empty() {
                        format!("{}::{}", const_prefix, cname)
                    ...
                    writeln!(
                        f,
                        "{}({}, {}, {} , [{}]);",
                        m_const_palette_u32_getter, fn_name, ffi_name, full, field_list
                    )?;
                }
            }
        }
    ...
    for (name, file) in by_type_consts.get("Color").cloned().unwrap_or_default() {
        ...
        let full = if mod_path.is_empty() {
            ...
            format!("{}::{}::{}", const_prefix, mod_path.replace('/', "::"), name)
        };
        let fn_name = if mod_path.starts_with("style::palette::tailwind") {
            format!(
                "{}_palette_tailwind_get_{}",
        ...
        writeln!(f, "{}({}, {});", m_const_color_u32_getter, fn_name, full)?;
    }
    ...
    let mut emit_const_module = |module: &str, file: &str| -> std::io::Result<()> {
        ...
        for line in module_text.lines() {
            ...
            match base_ty {
                "char" => {
                    writeln!(f, "{}({}, {});", m_const_char_getter, fn_name, full)?;
                }
                "u16" => {
                    writeln!(f, "{}({}, {});", m_const_u16_getter, fn_name, full)?;
                }
                _ => {}
            }
        }
    ...
    let mut emit_set_module = |module: &str, file: &str| -> std::io::Result<()> {
        ...
        for cname in names {
            ...
            writeln!(f, "{}({}, {}, {} , [{}]);", m_const_struct_getter, fn_name, ffi_name, full, fields)?;
        }
        Ok(())
    };
    ...
    // Lightweight cleanup pass: shorten prefixes for this repo
    if gen.const_root == "ratatui" {
        if let Ok(mut txt) = std::fs::read_to_string(out_path) {
            if !txt.contains("use ratatui as rt;") {
                let mut lines = txt.lines().collect::<Vec<_>>();
                if !lines.is_empty() {
                    let mut out = String::new();
                    out.push_str(lines[0]);
                    out.push('\n');
                    out.push_str("use ratatui as rt;\n");
                    for l in &lines[1..] { out.push_str(l); out.push('\n'); }
                    txt = out;
                }
            }
            txt = txt.replace("crate::ratatui_", "ratatui_");
            txt = txt.replace("ratatui::", "rt::");
            let _ = std::fs::write(out_path, txt);
        }
    }
    Ok(())
}
